...
contract QrisEscrowV2 is AccessControl, Pausable, ReentrancyGuard {
    ...
    // ============ Settlement Events (Fiat Conversion Tracking) ============
    
    /**
     * @notice Emitted when fiat settlement process is initiated
     * @param orderId The order being settled
     * @param merchantId Merchant identifier from QRIS
     * @param cryptoAmount Amount in crypto (LSK) being converted
     * @param fiatAmountIdr Expected fiat amount in IDR
     * @param exchangeRate Exchange rate used (IDR per 1 LSK, scaled by 1e8)
     */
    event SettlementInitiated(
        bytes32 indexed orderId,
        string merchantId,
        uint256 cryptoAmount,
        uint256 fiatAmountIdr,
        uint256 exchangeRate
    );
    
    /**
     * @notice Emitted when fiat has been sent to merchant's bank account
     ...
     * @param timestamp When the settlement was completed
     */
    ...
    // ============ Settlement Functions (Fiat Conversion Tracking) ============
    
    /**
     * @notice Record that fiat settlement has been initiated for an order
     * @dev Called by operator when crypto-to-fiat conversion starts
     * @param orderId The order being settled
     * @param fiatAmountIdr Expected fiat amount in IDR
     * @param exchangeRate Exchange rate used (IDR per 1 LSK, scaled by 1e8)
     */
    function recordSettlementInitiated(
        bytes32 orderId,
        uint256 fiatAmountIdr,
        uint256 exchangeRate
    ...
    /**
     * @notice Record that fiat has been sent to merchant's bank account
     * @dev Called by operator when bank transfer is confirmed
     * @param orderId The order that was settled
     * @param bankReference Bank transfer reference number
     * @param fiatAmountIdr Actual fiat amount sent to merchant
     */
    function recordSettlementCompleted(
        bytes32 orderId,
