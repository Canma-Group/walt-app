...
const ESCROW_V2_ABI = [
  ...
  // Settlement tracking functions (v2.1.0)
  'function recordSettlementInitiated(bytes32 orderId, uint256 fiatAmountIdr, uint256 exchangeRate) external',
  'function recordSettlementCompleted(bytes32 orderId, string bankReference, uint256 fiatAmountIdr) external',
  // Events
  ...
  'event SettlementInitiated(bytes32 indexed orderId, string merchantId, uint256 cryptoAmount, uint256 fiatAmountIdr, uint256 exchangeRate)',
  'event SettlementCompleted(bytes32 indexed orderId, string merchantId, string bankReference, uint256 fiatAmountIdr, uint256 timestamp)',
];
...
export class QrisEscrowV2Service {
   ...
   * @param paymentId - UUID payment ID (will be converted to orderId)
   */
  ...
  /**
   * Release payment by orderId directly (for orders from EscrowListener)
   * @param orderId - bytes32 order ID (already in correct format)
   */
  async releaseByOrderId(orderId: string): Promise<string> {
    console.log(`[QrisEscrowV2] Releasing by orderId: ${orderId}`);
  ...
  // ============ Settlement Tracking (On-chain Audit Trail) ============

  /**
   * Record settlement initiation on-chain
   * @param orderId bytes32 order ID
   * @param fiatAmountIdr Fiat amount in IDR (will be scaled to wei)
   * @param exchangeRate Exchange rate (IDR per 1 LSK)
   */
  async recordSettlementInitiated(
    orderId: string,
    fiatAmountIdr: number,
    exchangeRate: number
  ): Promise<string> {
    console.log(`[QrisEscrowV2] Recording settlement initiated for ${orderId}`);
    console.log(`  - Fiat: Rp ${fiatAmountIdr.toLocaleString()}`);
    console.log(`  - Rate: Rp ${exchangeRate.toLocaleString()} per LSK`);
    
    // Scale to wei (use 1e8 for precision)
    const fiatAmountScaled = BigInt(Math.round(fiatAmountIdr * 1e8));
    const exchangeRateScaled = BigInt(Math.round(exchangeRate * 1e8));
    
    const tx = await this.escrowContract.recordSettlementInitiated(
      orderId,
      fiatAmountScaled,
      exchangeRateScaled
    );
    ...
    console.log(`[QrisEscrowV2] SettlementInitiated recorded! TxHash: ${receipt.hash}`);
    return receipt.hash;
  }

  /**
   * Record settlement completion on-chain
   * @param orderId bytes32 order ID
   * @param bankReference Bank transfer reference number
   * @param fiatAmountIdr Actual fiat amount sent to merchant
   */
  async recordSettlementCompleted(
    orderId: string,
    bankReference: string,
    fiatAmountIdr: number
  ): Promise<string> {
    console.log(`[QrisEscrowV2] Recording settlement completed for ${orderId}`);
    console.log(`  - Bank Ref: ${bankReference}`);
    console.log(`  - Fiat: Rp ${fiatAmountIdr.toLocaleString()}`);
    
    // Scale to wei (use 1e8 for precision)
    const fiatAmountScaled = BigInt(Math.round(fiatAmountIdr * 1e8));
    
    const tx = await this.escrowContract.recordSettlementCompleted(
      orderId,
      bankReference,
      fiatAmountScaled
    );
    ...
    console.log(`[QrisEscrowV2] SettlementCompleted recorded! TxHash: ${receipt.hash}`);
    return receipt.hash;
  }
