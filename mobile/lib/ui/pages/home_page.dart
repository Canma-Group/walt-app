...
class _HomePageState extends State<HomePage> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (user != null) {
        ...
        if (walletAddr != null && walletAddr.isNotEmpty) {
          _subscribeToTransactions(walletAddr);
        }
      }
    });
  }
  ...
  void _subscribeToTransactions(String walletAddress) {
    _transactionSubscription?.cancel();
    _transactionSubscription = _transactionService.transactionsStream(walletAddress).listen(
      (transactions) {
        if (mounted) {
          setState(() {
            _recentTransactions = transactions.take(2).toList();
          });
          print('[HomePage] Real-time update: ${transactions.length} transactions, showing ${_recentTransactions.length}');
        }
      },
      onError: (error) {
        print('[HomePage] Transaction stream error: $error');
      },
    );
  }
  ...
  Future<void> _loadRecentTransactions(String? walletAddress) async {
    ...
    try {
      ...
      print('[HomePage] Loaded ${transactions.length} recent transactions');
      for (int i = 0; i < transactions.length; i++) {
        final tx = transactions[i];
        print('[HomePage] TX[$i]: ${tx.senderName} -> ${tx.receiverName}, ${tx.amount} ${tx.token}, created: ${tx.createdAt}');
      }
      if (mounted) {
        setState(() {
  ...
  Widget _buildTopUpOption({
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isComingSoon ? Colors.grey[100] : const Color(0xFFE8EEF6),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: isComingSoon ? Colors.grey[300]! : const Color(0xFF1264EF),
            width: 1.5,
          ),
        ),
        child: Row(
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          SizedBox(height: 100 * s), // Bottom padding for navbar
        ],
      ),
    );
  }
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(
                          walletAddressDisplay.isNotEmpty ? walletAddressDisplay : '0x...',
                          style: GoogleFonts.poppins(
                            color: Colors.white,
                            fontSize: 14 * s,
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(Icons.add, size: 14 * s, color: Colors.white),
                      SizedBox(width: 4 * s),
                      Text(
                        'Add New',
                        style: GoogleFonts.poppins(
    ...
    return Column(
      ...
      children: [
        ...
        if (_isLoadingTokens)
          ...
          SizedBox(height: 8 * s),
          // Ethereum
          _buildAssetItemWithIDR(s, 'ETH', 'Ethereum', _getTokenBalance('ETH'), const Color(0xFFECEFF0)),
        ],
      ],
    );
  }
    ...
    return Column(
      ...
      children: [
        ...
        if (_isLoadingTransactions)
          ...
          ..._recentTransactions.take(2).map((tx) => _buildRecentTxItem(tx, s)),
      ],
    );
  }
  ...
  Widget _buildRecentTxItem(TransactionModel tx, double s) {
    ...
    // Determine display info based on transaction type and direction
    String displayName;
    String subtitle;
    bool showAsReceived = !isSender;
    
    if (isSwap) {
      // For swap transactions, show "Token Swap" with swap details
      displayName = 'Token Swap';
      subtitle = '${tx.fromToken ?? tx.token} → ${tx.toToken ?? tx.token}';
      showAsReceived = true; // Show swap as received (green, positive)
    } else if (isSender) {
      // Show receiver name, or "External Wallet" if empty/unknown
      displayName = tx.receiverName.isNotEmpty && tx.receiverName != 'Unknown' && tx.receiverName != 'You'
          ? tx.receiverName 
          : 'External Wallet';
      subtitle = 'Sent';
    } else {
      // Show sender name
      displayName = tx.senderName.isNotEmpty && tx.senderName != 'Unknown' && tx.senderName != 'You'
          ? tx.senderName
          : 'External Wallet';
      subtitle = 'Received';
    }
    ...
    return Container(
      ...
      child: Row(
        children: [
          // Icon - pill style like My Assets
          Container(
            width: 50 * s,
            height: 50 * s,
            decoration: BoxDecoration(
              color: showAsReceived
                  ? Colors.green.withOpacity(0.1)
                  : Colors.red.withOpacity(0.1),
              shape: BoxShape.circle,
            ),
            child: Icon(
              isSwap ? Icons.swap_horiz : (showAsReceived ? Icons.arrow_downward : Icons.arrow_upward),
              color: showAsReceived ? Colors.green[600] : Colors.red[600],
              size: 24 * s,
            ),
          ),
          SizedBox(width: 12 * s),
          ...
          // Amount - right aligned like My Assets
          Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  '${showAsReceived ? '+' : '-'}${tx.formattedAmount}',
                  style: GoogleFonts.poppins(
                    fontSize: 14 * s,
                    fontWeight: FontWeight.w600,
                    color: const Color(0xFF3A3A3A),
      ...
      return Expanded(
        child: GestureDetector(
          ...
          child: Container(
            ...
            height: 69 * s,
            decoration: BoxDecoration(
              color: active ? const Color(0xFF1264EF) : Colors.white,
